object SyntacticConveniences {
  ...
  case class Range(start: Int, end: Int, step: Int = 1 /*add a default parameter, so the step is 1*/)


  def average(x: Int, xs: Int*/*add an expression for the repeated parameter*/): Double =
    (x :: xs.to[List]).sum.toDouble / (xs.size + 1)


  type Result = Either[String, (Int, Int) /*the output result of the divide should be a tuple of Ints*/]
