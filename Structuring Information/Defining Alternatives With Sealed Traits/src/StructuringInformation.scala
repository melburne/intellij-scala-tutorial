...
object StructuringInformation extends FlatSpec with Matchers {
  ...
  case class Rest(duration: String) extends Symbol //extend the sealed trait Symbol here

  def symbolDuration(symbol: Symbol): String =
    symbol match {
    case Note(name, duration, octave) => duration //we need to return a similar to the Rest field
